...
$LANG->includeLLFile('EXT:tmd_cinema/tx_tmdcinema_program_program/locallang.xml');
...
/**
 * tmd_cinema module tmd_cinema_program_programwiz0
 *
		...
		foreach($cfgArr as $cellArr)	{
			if (is_array($cellArr))	{
				...
				foreach($cellArr as $cellContent)	{
					if ($this->inputStyle)	{
						$cells[]='<input type="text"'.$this->doc->formWidth(8).' name="TABLE[c]['.(($k+1)*2).']['.(($a+1)*2).']" value="'.htmlspecialchars($cellContent).'" />';
					} else {
						$cellContent=eregi_replace('<br[ ]?[\/]?>',chr(10),$cellContent);
						$cells[]='<textarea '.$this->doc->formWidth(8).' rows="5" name="TABLE[c]['.(($k+1)*2).']['.(($a+1)*2).']">'.t3lib_div::formatForTextarea($cellContent).'</textarea>';
					}
...
#				$ctrl.='<input type="image" name="TABLE[col_add]['.(($a+1)*2).']"'.t3lib_iconWorks::skinImg($this->doc->backPath,'gfx/add.gif','').' title="'.$LANG->getLL('table_addColumn',1).'" />';
		...
		$content.= '
			...
			<div id="c-inputMode">
				'.
				'<input type="hidden" name="TABLE[textFields]" value="1" />
			</div>
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tmd_cinema/tx_tmdcinema_program_program/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tmd_cinema/tx_tmdcinema_program_program/index.php']);
}
